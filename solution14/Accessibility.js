document.addEventListener("DOMContentLoaded", (event) => {
    ...
    const invalidAnswer = document.getElementById("invalid-answer");
    ...
    function showQuestion() {
        ...
        answer01.textContent = questionObj.options[0];
        answer02.textContent = questionObj.options[1];
        answer03.textContent = questionObj.options[2];
        answer04.textContent = questionObj.options[3];
  
        selectedAnswerIndex = null;
        buttons.forEach(button => {
            button.querySelector('.answer').classList.remove("active", "right", "wrong");
            button.classList.remove("rightBorder", "wrongBorder");
            button.querySelector('#correct').style.display = 'none';
            button.querySelector('#wrong').style.display = 'none';
    ...
    buttons.forEach((button, index) => {
        ...
        button.addEventListener('click', function () {
            buttons.forEach(btn => {
                btn.querySelector('.answer').classList.remove('active');
            });
    ...
    btnSubmit.addEventListener("click", function () {
        if (waitingForNextQuestion) {
            ...
            if (currentQuestionIndex < quizzes.quizzes[currentQuizIndex].questions.length) {
                showQuestion();
            } else {
                score.style.display = 'block';
                quiz.style.display = 'none';
                scored.textContent = `${points}`;
            ...
            if (selectedAnswerIndex !== null) {
                ...
                if (questionObj.options[selectedAnswerIndex] === questionObj.answer) {
                    buttons[selectedAnswerIndex].classList.add('rightBorder');
                    buttons[selectedAnswerIndex].querySelector('#correct').style.display = 'block';
                    buttons[selectedAnswerIndex].querySelector('.answer').classList.remove('active');
                    buttons[selectedAnswerIndex].querySelector('.answer').classList.add('right');
                    points++;
                } else {
                    buttons[selectedAnswerIndex].querySelector('#wrong').style.display = 'block';
                    buttons[selectedAnswerIndex].querySelector('.answer').classList.add('wrong');
                    buttons[selectedAnswerIndex].classList.add('wrongBorder');
                    buttons[correctIndex].querySelector('.answer').classList.add('right');
                    buttons[correctIndex].querySelector('#correct').style.display = 'block';
                    buttons[correctIndex].classList.add('rightBorder');
                }
    
                if(currentQuestionIndex == 9){
                    btnSubmit.textContent = "Finish Quiz";
                } else {
                    btnSubmit.textContent = "Next Question";
                }
                waitingForNextQuestion = true;
            } else {
                invalidAnswer.style.display = 'flex';
            }
        }
    });
